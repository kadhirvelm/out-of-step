...
describe("can correctly increment the time", () => {
    it("can return a valid time", () => {
        const currentDate = dayjs("2020-12-15 15:00:00", "America/Los_Angeles");
        expect(getNextTimeWithinMarketHours(currentDate).valueOf()).toEqual(currentDate.valueOf());
    });

    it("can increment to the next day", () => {
        const currentDate = dayjs("2020-12-15 22:00:00", "America/Los_Angeles");
        expect(getNextTimeWithinMarketHours(currentDate).valueOf()).toEqual(
            dayjs("2020-12-16 06:00:00", "America/Los_Angeles").valueOf(),
        );
    });

    it("can increment to the start of the market", () => {
        const currentDate = dayjs("2020-12-15 03:00:00", "America/Los_Angeles");
        expect(getNextTimeWithinMarketHours(currentDate).valueOf()).toEqual(
            dayjs("2020-12-15 06:00:00", "America/Los_Angeles").valueOf(),
        );
    });

    it("can increment to the start of the next market day", () => {
        const currentDate = dayjs("2020-12-18 21:00:00", "America/Los_Angeles");
        expect(getNextTimeWithinMarketHours(currentDate).valueOf()).toEqual(
            dayjs("2020-12-21 06:00:00", "America/Los_Angeles").valueOf(),
        );
    });

    it("can increment to the start of the next market day take 2", () => {
        const currentDate = dayjs("2020-12-19 15:00:00", "America/Los_Angeles");
        expect(getNextTimeWithinMarketHours(currentDate).valueOf()).toEqual(
            dayjs("2020-12-21 06:00:00", "America/Los_Angeles").valueOf(),
        );
    });
